...
--## APPS
--#####################################################
--# NOTE: exec commands below (android-studio, dataspell, drawio, figma-linux,
--# pycharm, idea, telegram-desktop, nextcloud, discord, stremio, notion-app,
--# obsidian, nwg-displays) are common package/launcher names — replace any
--# that don't match how the app is actually launched on your system
--# (check with `which <cmd>` or the app's .desktop file Exec= line).
...
hl.bind("SUPER + CTRL + S", hl.dsp.exec_cmd("stremio"), { description = "App: Stremio" })
...
--#####################################################
--## SHELL / QUICKSHELL (search, sidebars, overlays)
--#####################################################
...
hl.bind("SUPER + SHIFT + N", hl.dsp.global("quickshell:sidebarRightToggle"), { description = "Shell: Toggle right sidebar (notifications)" })
...
--# Send to workspace left/right (scroll / page keys)
for i = 1, 4 do
...
for i = 1, 2 do
    local keys = { "Left", "Right" }
    local prefix = { "m-", "m+" }
    hl.bind("CTRL + SUPER + ALT + " .. keys[i], hl.dsp.focus({ workspace = prefix[i] .. "1" })) -- # [hidden] Focus busy left/right
end
for i = 1, 4 do
...
for i = 1, 4 do
    ...
    local keycombos = { key[1], key[2], "CTRL + " .. key[1], "CTRL + " .. key[2] }
    local prefix = { "+", "-", "r+", "r-" }
    hl.bind(keycombos[i], hl.dsp.focus({ workspace = prefix[i] }))
end
...
--#####################################################
--## TESTING / HIDDEN (dev-only, safe to remove)
--#####################################################
